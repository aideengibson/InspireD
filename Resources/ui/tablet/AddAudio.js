...
//todo if audio file hasn't been saved, cannot save/ delete or go back
function AddAudioWindow(title){

    var addAudioWindow = Ti.UI.createWindow({
...
recording.format = Ti.Media.AUDIO_FILEFORMAT_CAF;
	...
	btnLeft.addEventListener('click',function(e){
			...
			textfieldAudio.value = '';  
			newPhoto.image = '';
		//	insertAuditlog(Ti.App.Properties.getString('currentUserType'), Ti.App.Properties.getString('currentDevice'), title+'Add New Audio Cancelled', GetDateTime(), 'N');
   		 	}
   		 	else {
 ...
 function deleteAudioFile(fname){
 	Ti.API.info('filename: '+fname+' directoryname: '+photodir);
	...
	if (!audioDirectory.exists()){
		...
		if (filetodelete.exists()){
			filetodelete.deleteFile();	
			insertAuditlog(Ti.App.Properties.getString('currentUserType'), Ti.App.Properties.getString('currentDevice'), title+'Audio Deleted', GetDateTime(), 'N');
			Ti.API.info('Audio deleted: '+filetodelete);
    	}
   		 else{
   // no audio file to delete so do nothing
	...
	function saveDatatoDB(filename){
		insertAudio(filename, textfieldAudio.value);
	};
...
function addPhoto(){		
	...
	textfieldAudio.value = '';   
	newPhoto.image = '';
	insertAuditlog(Ti.App.Properties.getString('currentUserType'), Ti.App.Properties.getString('currentDevice'), title+'New Audio Added', GetDateTime(), 'N');
  	savedDialog.show();
  	labelLeft.text = 'Delete';
};
	...
	btnRight.addEventListener('click',function(e){
		...
		//	insertAuditlog(Ti.App.Properties.getString('currentUserType'), Ti.App.Properties.getString('currentDevice'), title+'Close Add Audio. Back to Viewer', GetDateTime(), 'N');
			picturesRetrieved = 'no';
			addAudioWindow.close();
  			}
		else {
   ...
   var deletedDialog = Titanium.UI.createOptionDialog({
	...
	{	
    	...
    	if(e.index == 0)
    	{	
    		var phototodelete = getAudio(photodata[0].image);
    		var allAudios = getAudios();
    		Ti.API.info('Audio array length: '+(allAudios.length).toString());
    		Ti.API.info('Found filename: '+phototodelete[0].id);
   		 	deleteImage(rawfilename);
   		 	deleteAudioFile(rawfilename);
   		 	deleteAudio(phototodelete[0].id);
   		 	var allAudios = getAudios();
   		 	
    		Ti.API.info('Audio array after delete: '+(allAudios.length).toString());
   		 	photodata = [];
    		textfieldAudio.value = '';
    		rawfilename = '';
        	newPhoto.image = ''; 
...
if (b1.title == "Stop")
	...
	btnCentre.addEventListener('click',function(e){// exit photos
		//show dialog to record
  		...
  		if (rawfilename == '') {	
  			alert('Please Add an image to remember this recording');
  		
  		}
  		else {
  			if (textfieldAudio.value == '') {  
  				alert('Please Add a title to remember this recording');
  			}
  			else {
	...
	addAudioWindow.titleControl = header;
	addAudioWindow.add(subheader);
	addAudioWindow.add(detailview);
...
var row1 = Ti.UI.createTableViewRow({
	...
	var labelAudio = Titanium.UI.createLabel({
		top: 0,
		backgroundColor: 'transparent',
		color: Ti.App.Properties.getString('appfontcolor'),
	...
	var textfieldAudio = Titanium.UI.createTextField({
	...
	{ 
		if (phototosave == 'yes'){
			if (imagetosave == 'images/emptyimage.png' || rawfilename == '')  {
				...
				if (audiotosave != ''){// an audio file has been added
					if (textfieldAudio.value != ''){ //the audio has been given a title
						addPhoto();
						Ti.API.info('Add photo called');
					}
					else {
						alert('Please give sound a title!')
					};
				}
				else
				{
					alert('Please record a sound!')
				};
			};
		}
		else {
...
detailview.add(detailSubview);	
	...
	try {
		detailSubview.removeAllChildren();
 		...
 		Ti.API.info('Error removing child views');
 	};
	...
	if (filetodisplay.exists()){
        ...
        photodata = getAudio(res);
        rawfilename = res;
       textfieldAudio.value = photodata[0].title;
    ...
    buttonGetPhoto.addEventListener('click',function(e){
    	if (phototosave == 'no'){
			...
			if (imagetosave == 'images/emptyimage.png' || rawfilename == '')  {
				...
				if (audioSubview.visible == true) {
				//  Save audio file and photo to file and save details to DB
				addPhoto();
				picturesRetrieved = 'no';
				getPictures();
				}
				else {
					alert('Please record an audio clip to save!');  
				}
			};
		};
    });
    
    addAudioWindow.addEventListener('focus', function(e){
		Ti.API.info('focus on Add Audios');
