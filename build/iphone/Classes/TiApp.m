...
- (NSString*)systemUserAgent
{
    ...
    NSString *kInspiredUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    
    return [[NSString stringWithFormat:@"%@/%s (%@)",kInspiredUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Inspired app using the node.js CLI
