...
//Rare exceptions to indicate a bug in the _inspired code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_inspired = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_inspired]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.inspired.module"
