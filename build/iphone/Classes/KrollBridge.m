...
NSString * Inspired$ModuleRequireFormat = @"(function(exports){"
...
{
	// create Inspired global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "Inspired" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_inspired = [[InspiredObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_inspired];

	NSString *_inspiredNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _inspiredNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_inspired valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Inspired$ModuleRequireFormat, dirname, filename, code];
...
- (TiModule *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.inspired.kroll"
